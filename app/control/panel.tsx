    ...
    <div className="min-h-dvh bg-page p-6">
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4">
        <header className="flex items-baseline gap-3">
          <span className="text-[17px] font-bold text-ink">Handoff</span>
          <span className="text-sm text-muted">demo control — not part of the product demo</span>
        </header>

        <section className="rounded-xl border border-line bg-surface p-4">
          <div className="mb-2 text-[11px] uppercase tracking-wide text-muted">Access</div>
          <input
            type="password"
            value={key}
            onChange={(e) => saveKey(e.target.value)}
            placeholder="HERMES_SECRET (blank in local dev)"
            className="w-full rounded-md border border-dashed border-line-strong bg-page px-3 py-2 text-[13px] font-mono text-ink placeholder:text-muted"
          />
        ...
        <section className="rounded-xl border border-line bg-surface p-4">
          <div className="mb-2 flex items-baseline gap-3">
            <span className="text-[11px] uppercase tracking-wide text-muted">Engine clock</span>
            {status && (
          ...
          <div className="mb-3 flex items-baseline gap-4 font-mono tabular-nums">
            <div>
              ...
              <div className="text-[11px] uppercase tracking-wide text-muted">engine</div>
            </div>
            <div>
              <div className="text-[21px] text-ink-soft">
                {status ? new Date(status.real).toLocaleTimeString([], { hour: "numeric", minute: "2-digit", second: "2-digit" }) : "—"}
              </div>
              <div className="text-[11px] uppercase tracking-wide text-muted">wall</div>
            </div>
          </div>
          <div className="flex flex-wrap gap-1.5">
            {SPEEDS.map((s) => (
              <button
                ...
                onClick={() => setSpeed(s)}
                disabled={busy !== null}
                className={`rounded-full border px-3 py-1.5 text-[13px] font-semibold disabled:opacity-40 ${
            ...
            <span className="mx-1 self-center text-[12px] text-muted">·</span>
            {JUMPS.map((h) => (
              <button
                key={h}
                onClick={() => jump(h)}
                disabled={busy !== null}
                className="rounded-full border border-line-strong px-3 py-1.5 text-[13px] text-ink-soft disabled:opacity-40"
              >
                +{h}h
              </button>
            ))}
            <button
              onClick={clockReset}
              disabled={busy !== null}
              className="ml-auto rounded-full border border-line-strong px-3 py-1.5 text-[13px] text-ink-soft disabled:opacity-40"
            >
              Back to real time
            </button>
          </div>
        ...
        <section className="rounded-xl border border-line bg-surface p-4">
          <div className="mb-2 text-[11px] uppercase tracking-wide text-muted">Hermes heartbeat</div>
          <div className="flex items-center gap-3">
            <button
              onClick={doTick}
              disabled={busy !== null}
              className="rounded-md bg-brand px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-40"
            >
          ...
          {lastTick && (
            <div className="mt-3 rounded-md bg-page px-3 py-2.5 font-mono text-[13px] leading-relaxed text-ink">
        ...
        <section className="rounded-xl border border-critical/40 bg-surface p-4">
          <div className="mb-2 text-[11px] uppercase tracking-wide text-critical">Danger</div>
          <div className="flex items-center gap-3">
            <button
              onClick={resetWorld}
              disabled={busy !== null}
              className="rounded-md border-[1.5px] border-critical px-4 py-2.5 text-sm font-semibold text-critical disabled:opacity-40"
            >
              Reset world
            </button>
            <div className="text-[12px] leading-relaxed text-muted">
          ...
          {lastReset && (
            <div className="mt-3 rounded-md bg-page px-3 py-2.5 font-mono text-[13px] text-ink">
              {lastReset}
            </div>
          )}
        </section>
        ...
        {error && (
          <div className="rounded-md border border-critical/40 bg-surface px-3 py-2.5 text-[13px] text-critical">
            {error}
          </div>
        )}
      </div>
