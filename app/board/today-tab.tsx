  ...
  return (
    <div className="p-5 bg-cream">
      <div className="rounded-2xl border border-line bg-surface p-5">
        <div className="mb-1 text-[17px] font-semibold text-ink">Today</div>
        ...
        {empty && (
          <div className="rounded-xl border border-dashed border-line-strong py-10 text-center text-xs text-muted">
            ...
            {proposals.map((p) => (
              <Pulse key={p.id} watch={p.detail} className="rounded-xl border border-critical/30 bg-page p-3.5">
                <div className="flex items-baseline gap-2">
                  <span className="rounded-full bg-critical px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-white">
                    Act now
                  </span>
                  {p.context && <span className="text-[11px] font-semibold text-ink">{p.context}</span>}
                </div>
                <div className="my-1.5 text-[11.5px] font-medium leading-relaxed text-ink">{p.detail}</div>
                <div className="flex gap-1.5">
                  ...
                  <button
                    ...
                    className="ml-auto rounded-md border border-line-strong px-2.5 py-1.5 text-[10px] text-ink-soft"
                  >
                    Open on the board →
                  </button>
                </div>
          ...
          <Section label="Review & send" sub="Claude drafted these — your words go out, not the AI's">
            {drafts.map((d) => (
              <div key={d.id} className="rounded-xl border border-warning/40 bg-page p-3.5">
                <div className="flex items-baseline gap-2">
                  <span className="rounded-full bg-warning px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wide text-ink">
                    Family message
                  </span>
                  {d.context && <span className="text-[11px] font-semibold text-ink">{d.context}</span>}
                </div>
          ...
          <Section label="Watch" sub="flagged before they're late — no decision needed yet">
            {watches.map((o) => (
              ...
              >
                ...
                <span className="text-[11px] font-medium text-ink">{o.patientLabel}</span>
  ...
  return (
    <div className="mb-4">
      <div className="mb-2 flex items-baseline gap-2">
        <span className="text-[10px] font-semibold uppercase tracking-wide text-ink">{label}</span>
...
function StatTile({
  ...
  return (
    <div className="rounded-xl border border-line p-3">
      ...
      <div
        className={`mt-0.5 text-xl font-semibold tabular-nums ${critical ? "text-critical" : "text-ink"}`}
      >
        {value}
      </div>
    </div>
